...
| **3b.4** | Add `_ssl_capability.RAND_bytes`, `RAND_priv_bytes`, `OPENSSL_VERSION`, and `load_default_certs()` that installs the bundled Mozilla WebPKI roots. CA bundle pinned at fetch time (cpython-ext/_ssl/ca-bundle/cacert.pem, SHA-256 in ca_bundle.h header). | `make test-ssl-network` passes 18 assertions including CERT_REQUIRED-rejects-expired-cert + CERT_REQUIRED-rejects-self-signed + CERT_NONE-still-accepts-same-expired-cert (proves gate is real). 121 CAs loaded. | **✅ DONE**. |
| **3b.5** | Wire the socket-backed path: `_SSLSocket` over a real fd. Under wasmtime this is automatic — `openssl:component/tls.client.connect(host, port, config)` handles the socket itself via wasi:sockets/tcp. | Collapsed into 3b.3 (openssl-component does this internally). | **✅ DONE**. |
| **3b.6** | Provide a Python-level wrapper so existing code can transparently use the capability TLS. **Decision (vs plan):** rather than monkey-patch every `_ssl` symbol (CPython's `ssl.py` imports 30+; too fragile for v1), ship `ssl_capability.py` as a separate module exposing a stdlib-shaped surface (SSLContext / SSLSocket / create_default_context / RAND_bytes / Purpose / errors). Users opt in with `import ssl_capability as ssl`, or patch `ssl.SSLContext = ssl_capability.SSLContext` for `urllib.request` compatibility. Phase 5 retires static `_ssl` and ssl.py gets fully re-routed. | `make test-ssl-network` shows `urllib.request.urlopen('https://example.com')` returns 200 OK with body containing "Example Domain", via patched ssl.SSLContext. 21 total assertions. | **✅ DONE**. |
