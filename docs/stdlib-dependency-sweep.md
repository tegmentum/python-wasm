...
| `ssl` | TLS handshake, cert validation, urllib.urlopen, MemoryBIO, **SSLObject (wrap_bio)**, **SSLSession (TLS 1.3 resumption)**, **peer chain (intermediates)**, **parsed `getpeercert(binary_form=False)`** (subject/issuer/version/serial/notBefore/notAfter/subjectAltName via cap `x509.certificate.info`), `get_server_certificate`, DER↔PEM, RAND_add/status, `cert_time_to_seconds`, truststore subclassing | _no openssl-component gaps left as of 2026-05-30; only server-side `wrap_socket(server_side=True)` remains for future work._ |
