...
| `ssl` | TLS handshake, cert validation, urllib.urlopen, MemoryBIO, **SSLObject (wrap_bio)**, **SSLSession (TLS 1.3 resumption)**, **peer chain (intermediates)**, **parsed `getpeercert(binary_form=False)`** (subject/issuer/version/serial/notBefore/notAfter/subjectAltName via cap `x509.certificate.info`), **server-side TLS** (cap-native `ctx.bind_tls(host, port).accept()` returns server `SSLSocket`; verified end-to-end against host `curl --insecure`), `get_server_certificate`, DER↔PEM, RAND_add/status, `cert_time_to_seconds`, truststore subclassing | _no openssl-component gaps left as of 2026-05-30; stdlib `wrap_socket(sock, server_side=True)` raises NotImplementedError pointing to `bind_tls()` (cap owns its socket lifetimes, can't accept a stdlib fd handoff)._ |
| `zlib` / `gzip` browser path | `_zlib_cap.deflate_compress` + the full Lib/zlib.py shim — works in both wasmtime and browser as of 2026-05-30 after the wac-cli 0.10 fix | (nothing) | Was broken on wac-cli 0.8.1 (resource-method canonical-ABI bug); compose-python-component.sh now version-gates wac>=0.10.0. |
