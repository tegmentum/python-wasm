...
| `ssl` | TLS handshake, cert validation, urllib.urlopen, MemoryBIO, **SSLObject (wrap_bio)**, **SSLSession (TLS 1.3 resumption)**, **peer chain (intermediates)**, `get_server_certificate`, DER↔PEM, RAND_add/status, `cert_time_to_seconds`, truststore subclassing | _no openssl-component gaps left as of 2026-05-29; only synthetic `getpeercert(binary_form=False)` (real dict) and server-side `wrap_socket(server_side=True)` remain._ |
